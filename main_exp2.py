...
def get_distance_squared(one_center, one_set):
    sum_all = 0
    if len(one_center) != 0 and len(one_set) != 0:
        for i in range(num_features):
            base = one_set[i] - one_center[i]
            sum_all += math.pow(base, 2)
    return sum_all
...
def update_centers(the_clustered_sets, the_train_features, the_cluster_centers):
    temp = []
    for i in range(k_means):
        if len(the_clustered_sets[i]) == 0:
            one_center_temp = []
            for j in range(len(the_cluster_centers[i])):
                one_center_temp.append(the_cluster_centers[i][j])
            temp.append(one_center_temp)
        else:
...
def stop_condition(the_pre_centers, the_cur_centers):
    ...
    for i in range(k_means):
        ...
        if math.sqrt(get_distance_squared(the_pre_centers[i], the_cur_centers[i])) > 1:
            return False
    return True
...
def train(the_train_features, the_train_labels):
    ...
    while count_for < num_runs:
        ...
        while center_bool == True:
            ...
            cluster_centers = update_centers(clustered_sets, the_train_features, cluster_centers)
            distances = wrapper_get_distance_squared(the_train_features, cluster_centers)
            clustered_sets = wrapper_find_min_index(distances)
...
def print_as_pgm(one_cluster_center, the_index):
    ...
    for i in range(0, width*height):
        value = int(one_cluster_center[i]) * 16
        if value == 256:
            value = 255
        buff.append(value)

    # open file for writing
    filename = str(the_index) + "_exp2" + ".pgm"
